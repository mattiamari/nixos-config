...
in
{
  ...
  services.borgbackup.jobs = lib.mapAttrs (name: opts: {
    ...
    encryption = {
      mode = "repokey-blake2";
      passCommand = "cat ${config.homeserver.secretsDir}/borg-${name}";
    };
  ...
  services.restic.backups.everything =
    let
      b = config.services.borgbackup.jobs;
    in
    {
      repository = "/mnt/backupb/everything/restic-everything";
      passwordFile = "${config.homeserver.secretsDir}/restic-everything";
      timerConfig = null;
      progressFps = 0.1;
